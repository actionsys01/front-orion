...
import FilterModal from '@components/Filtros-Controle-Entrada/FilterModal';
import { useControlFilter } from '@contexts/ControlFilter';
import RadioFilter from '@components/Filtros-Controle-Entrada/FilterRadio';
...
export default function ControleEntrada() {
  ...
  const handleApproval = useCallback(async id => {
    ...
    setExitDate(
      data.data_saida === null ? new Date() : new Date(data.data_saida),
    );
  ...
  const gatheredData = useMemo(() => {
    const allData: any = [];
    if (entrance) {
      const sliceNullControl = entrance.filter(
        item => item.controle_entrada != null,
      );
        ...
        allData.push({
              ...
              content={[
                {
                  ...
                  onClick: !entrancePermissions.AUTORIZAR
                    ...
                    ? () => {
                        setModalStatus('autorizar'), modalVisibleHandler();
                      }
                    : item.controle_entrada?.status === 0
                    ? () => handleApproval(item.controle_entrada.id)
                    : item.controle_entrada?.status === 4
                    ? () => {
                        setModalStatus('exception'), modalVisibleHandler();
                      }
                    : () => '',
                  className: entrancePermissions.AUTORIZAR
                    ? 'able'
                    : 'disabled',
                },
                {
                ...
                },
                {
                  ...
                  onClick: !entrancePermissions.CANCELAR
                    ...
                    : item.controle_entrada?.status === 4
                    ? () => {
                        setModalStatus('cancelar'), modalVisibleHandler();
                      }
          ...
          status:
            item.controle_entrada?.status === 0
              ...
              : null,
          arrivalDate:
            item.controle_entrada === null
              ? format(new Date(item.data_entrada), 'dd/MM/yyyy')
              : format(
                  new Date(item.controle_entrada?.data_entrada),
                  'dd/MM/yyyy',
                ),
          exitDate:
            item.controle_entrada?.data_saida != null
              ? format(
                  new Date(item.controle_entrada?.data_saida),
                  'dd/MM/yyyy',
                )
              : '',
          arrivalTime:
            item.controle_entrada === null
              ? format(new Date(item.data_entrada), 'HH:mm')
              : format(new Date(item.controle_entrada?.data_entrada), 'HH:mm'),
          exitTime:
            item.controle_entrada?.data_saida != null
              ? format(new Date(item.controle_entrada?.data_saida), 'HH:mm')
              : '',
          emptyWeight:
            Number(item.controle_entrada?.peso_vazio) === 0
              ? ''
              : item.controle_entrada?.peso_vazio,
        });
      });
    }
    ...
    <>
      ...
      <BtnRow>
        ...
        <div>
          ...
          <button
            type="button"
            disabled={!entrancePermissions.ADICIONAR}
